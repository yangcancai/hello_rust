use std::{io::{self, BufReader}, path::Path, fs::File, sync::Arc};
use anyhow::Result;
use tokio::{net::TcpListener, io::{sink, AsyncWriteExt, copy, AsyncReadExt}};
use tokio_rustls::{rustls::{self, Certificate, PrivateKey}, TlsAcceptor};
...
async fn main() -> Result<()> {
    ...
    with_safe_defaults().
    with_no_client_auth().
    with_single_cert(certs, keys.remove(0)).map_err(|err| 
        io::Error::new(io::ErrorKind::InvalidInput, err))?;
    let acceptor = TlsAcceptor::from(Arc::new(config));
    let listener = 
    TcpListener::bind("127.0.0.1:5666").await?;
     loop{
